...
namespace Liberu\Ecommerce\Catalog\Filament\Tests;

use Filament\Facades\Filament;
use Liberu\Ecommerce\Catalog\CatalogServiceProvider;
use Liberu\Ecommerce\Catalog\Filament\Tests\Fixtures\TestPanelProvider;
 ...
 * *direct* requirements, which for `filament/filament` is a single provider. A
 * panel needs the rest of the stack — support, schemas, forms, tables, actions,
...
abstract class TestCase extends PackageTestCase
{
    ...
    /**
     * An actor working in a team, which is the whole of both policies.
     *
     * `current_team_id` is set in memory rather than migrated onto `users`: the
     * column belongs to the application's own schema, and a package that added
     * it to the shared testbench table would be asserting against a users table
     * no deployment has.
     */
    ...
    protected function defineEnvironment($app): void
    {
        ...
        // Nothing here loads a product's team through the relation, but the
        // default names the host's `App\Models\Team` — a class no package tree
        // has — and pointing it somewhere real keeps an accidental eager load
        // from failing on a missing class rather than on the thing under test.
        $app['config']->set('catalog.team_model', TestUser::class);

        // Left unset on purpose, which is the state every deployment that runs
        // the catalogue without Commerce Core is in. Nothing in this package may
        // touch `ProductPublication::channel()`, and a suite that configured a
        // channel model would stop proving it.
        $app['config']->set('catalog.channel_model', null);
    }
    ...
    protected function getPackageProviders($app): array
    {
        return array_values(array_unique([
            ...
            // tables to query without the migrations it loads and no policies
            // without the gates it binds, so the dependency is stated here too
